...
def test_oversold_bounce_total_failure_does_not_mark_success():
    """Every ticker raising a domain error -> no last-success bump; duration observed."""
    import asyncio
    from datetime import date
    from unittest.mock import AsyncMock, MagicMock, patch
    ...
    db = MagicMock()
    db.query.side_effect = ScanError("boom")

    with (
        patch("app.services.oversold_bounce_scan.scanner_events_total"),
        patch("app.services.oversold_bounce_scan.scan_duration_seconds") as mock_dur,
        patch(
            "app.services.oversold_bounce_scan.scan_last_success_timestamp"
        ) as mock_ts,
        patch(
        ...
        asyncio.run(
            run_oversold_bounce_scan(["AAA"], db=db, event_date=date(2026, 1, 15))
        )
