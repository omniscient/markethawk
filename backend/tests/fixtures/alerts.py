...
def seed_alert_delivery_logs(
    ...
    specs = [
        (rules[0], "AAPL", "pre_market_volume_spike", "browser_push", "sent", None, now),
        (rules[0], "MSFT", "pre_market_volume_spike", "browser_push", "sent", None, now - timedelta(hours=1)),
        (rules[1], "NVDA", "pre_market_volume_spike", "email", "sent", None, now - timedelta(hours=2)),
        (rules[1], "MRNA", "liquidity_hunt", "google_chat", "failed", "Webhook timeout", now - timedelta(hours=3)),
        (rules[2], "BNTX", "liquidity_hunt", "webhook", "sent", None, now - timedelta(hours=4)),
        (rules[2], "AAPL", "liquidity_hunt", "webhook", "failed", "Connection refused", now - timedelta(hours=5)),
    ]
    logs = []
