...
class AutoTradeOrder(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "symbol", "trading_strategy_id", "event_date",
    ...
    # ── Source linkage ───────────────────────────────────────────────────
    alert_rule_id       = Column(Integer, ForeignKey("alert_rules.id", ondelete="SET NULL"), nullable=True, index=True)
    scanner_event_id    = Column(Integer, ForeignKey("scanner_events.id", ondelete="SET NULL"), nullable=True, index=True)
    trading_strategy_id = Column(Integer, ForeignKey("trading_strategies.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    trigger_price      = Column(Numeric, nullable=True)  # price in event.indicators at alert moment
    entry_price_target = Column(Numeric, nullable=True)  # trigger adjusted for limit_offset_pct
    ...
    # ── Relationships ────────────────────────────────────────────────────
    trading_strategy = relationship("TradingStrategy", back_populates="auto_trade_orders")
