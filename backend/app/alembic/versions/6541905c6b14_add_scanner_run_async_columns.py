...
def upgrade() -> None:
    op.add_column('scanner_runs', sa.Column('scan_start_date', sa.Date(), nullable=True))
    op.add_column('scanner_runs', sa.Column('scan_end_date', sa.Date(), nullable=True))
    op.add_column('scanner_runs', sa.Column('celery_task_id', sa.String(length=64), nullable=True))
    op.create_index(op.f('ix_scanner_runs_celery_task_id'), 'scanner_runs', ['celery_task_id'], unique=False)
