...
def upgrade() -> None:
    ...
    op.add_column(
        "stock_splits",
        sa.Column(
            "source", sa.String(length=20), nullable=False, server_default="polygon"
        ),
    )
    op.add_column(
        "stock_splits", sa.Column("detected_at", sa.DateTime(), nullable=True)
    )
    op.add_column(
        "stock_splits",
        sa.Column("adjustments_applied_at", sa.DateTime(), nullable=True),
    )
    op.create_unique_constraint(
        "uq_split_ticker_date", "stock_splits", ["ticker", "execution_date"]
    )
