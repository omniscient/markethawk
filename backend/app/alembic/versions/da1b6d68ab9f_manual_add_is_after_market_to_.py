...
def upgrade() -> None:
    op.add_column(
        "stock_aggregates",
        sa.Column(
            "is_after_market", sa.Boolean(), nullable=True, server_default="false"
        ),
    )
    op.create_index(
        op.f("ix_stock_aggregates_is_after_market"),
        "stock_aggregates",
        ["is_after_market"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(
        op.f("ix_stock_aggregates_is_after_market"), table_name="stock_aggregates"
    )
