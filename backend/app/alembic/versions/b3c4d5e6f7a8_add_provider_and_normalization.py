...
def downgrade() -> None:
    op.execute(
        "ALTER TABLE universe_quality_reports DROP COLUMN IF EXISTS normalization_data"
    )
    op.execute(
        "ALTER TABLE universe_quality_reports DROP COLUMN IF EXISTS normalization_status"
    )
