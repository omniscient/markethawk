...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "enhance_suggestion", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
    ...
    op.create_index(
        op.f("ix_signal_reviews_id"), "signal_reviews", ["id"], unique=False
    )
    op.create_index(
        op.f("ix_signal_reviews_scanner_event_id"),
        "signal_reviews",
        ["scanner_event_id"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(
        op.f("ix_signal_reviews_scanner_event_id"), table_name="signal_reviews"
    )
