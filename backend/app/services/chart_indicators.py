...
class ChartIndicatorsService:
    ...
    def add_indicators(df: pd.DataFrame, is_intraday: bool = True) -> pd.DataFrame:
        ...
        df["isOpenOrClose"] = ((time_num >= 93000) & (time_num <= 93500)) | (
            (time_num >= 155900) & (time_num <= 160500)
        )
        ...
        df["swipe"] = (
            (~df["isRefVolumeZero"])
            & df["isMinimumVolumeAverage"]
            & (df["Volume"] > marketVolumeFactor * df["fastVolumeAverage"])
            & (df["ATR_1"] > marketAtrSwipeFactor * df["ATR_3"].shift(3))
        ...
        df["flush"] = (
            ...
            & df["isMinimumVolumeAverage"]
            & (df["Volume"] > marketVolumeFactor * df["fastVolumeAverage"])
            & (df["ATR_1"] > marketAtrFlushFactor * df["ATR_2"].shift(5))
        ...
        df["isPreMarketSwipe"] = (
            df["isMinimumVolumeAverage"]
            & (df["Volume"] > preMarketVolumeFactor * df["fastVolumeAverage"])
            & (df["ATR_1"] > preMarketAtrFactor * df["ATR_3"].shift(3))
            & (df["High"] >= df["High"].shift(1))
            & (df["High"] >= df["HHV_10"])
            & (~df["isMarketHours"])
            & (~df["isGhostPrint"])
        ...
        df["isPreMarketFlush"] = (
            df["isMinimumVolumeAverage"]
            & (df["Volume"] > preMarketVolumeFactor * df["fastVolumeAverage"])
            & (df["ATR_1"] > preMarketAtrFactor * df["ATR_2"].shift(5))
            & (df["Low"] <= df["Low"].shift(1))
            & (df["Low"] <= df["LLV_10"])
            & (~df["isMarketHours"])
            & (~df["isGhostPrint"])
        ...
        df["shouldPrintDownTriangle"] = (
            df["isMarketSwipe"] | df["isPreMarketSwipe"]
        ) & ~(
            (df["isPreMarketFlush"] | df["isMarketFlush"]) & (df["Close"] > df["Open"])
        )
        df["shouldPrintUpTriangle"] = (
            df["isMarketFlush"] | df["isPreMarketFlush"]
        ) & ~(
            (df["isPreMarketSwipe"] | df["isMarketSwipe"]) & (df["Open"] > df["Close"])
        )

        df["isHighVolTri"] = (
            df["isHighestVolumeOfDay"] & df["isMarketHours"] & (~df["isOpenOrClose"])
        )
        ...
        cols_to_drop = [
            ...
            "LLV_10",
            "HHV_Vol_Today",
            "isHighestVolumeOfDay",
            "isMarketHours",
            "isGhostPrint",
            "isOpenOrClose",
            "isMinimumVolumePerCandle",
            "isMinimumVolumeAverage",
            "Ref_V_1",
            "Ref_V_2",
            "Ref_V_3",
            "Ref_V_4",
            ...
            "isPreMarketSwipe",
            "isPreMarketFlush",
            "shouldPrintDownTriangle",
            "shouldPrintUpTriangle",
            "isHighVolTri",
        ]
