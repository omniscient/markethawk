...
class CatalystParser:
    ...
    PATTERNS = {
        "dilution_warning": re.compile(
            r"\b(offering|direct offering|public offering|shelf|atm|at-the-market|s-1|s-3|secondary|placement|warrants?)\b",
            re.IGNORECASE,
        ),
        "earnings_beat": re.compile(
            r"\b(beats?|exceeds?|tops?|surpasses?)\s+(eps|estimates|expectations|revenue|guidance)\b",
            re.IGNORECASE,
        ),
        "earnings_miss": re.compile(
            r"\b(missed?|misses?|cuts?|lowers?|reduces?)\s+(eps|estimates|expectations|guidance|forecast)\b",
            re.IGNORECASE,
        ),
        "fda_news": re.compile(
            r"\b(fda|approval|pdufa|fast track|orphan drug|clinical trial|phase [123]|ind|nda)\b",
            re.IGNORECASE,
        ),
        "merger_acquisition": re.compile(
            r"\b(merger|acquisition|acquires?|buyout|merges?|tender offer)\b",
            re.IGNORECASE,
        ),
        "analyst_upgrade": re.compile(r"\b(upgrades?|upgraded?)\b", re.IGNORECASE),
        "analyst_downgrade": re.compile(
            r"\b(downgrades?|downgraded?)\b", re.IGNORECASE
        ),
        "contract_won": re.compile(
            r"\b(awarded?|wins?|secures?)\b.*\b(contract|grant|award)\b", re.IGNORECASE
        ),
    }
    ...
    def batch_analyze(
        ...
        results = {
            t.upper(): {"tags": [], "summary": None, "latest_article_utc": None}
            for t in tickers
        }
        ...
        for ticker, recent_news in ticker_news.items():
            ...
            for article in recent_news:
                ...
                if (
                    "earnings" in content.lower()
                    and "earnings_beat" not in tags
                    and "earnings_miss" not in tags
                ):
                    tags.add("earnings")
