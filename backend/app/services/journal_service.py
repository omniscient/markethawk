...
from app.models.trade import Trade, TradeExecution, Tag, JournalEntry, trade_tags
from app.schemas.journal import TradeUpdate, TradeCreate, JournalEntryCreate, TagCreate, TradeStats
...
def get_trade_stats(db: Session) -> TradeStats:
    ...
    gross_profit = sum([t.net_pnl for t in trades if t.net_pnl and t.net_pnl > 0]) or Decimal("1")
    gross_loss = abs(sum([t.net_pnl for t in trades if t.net_pnl and t.net_pnl < 0])) or Decimal("1")
...
def import_trades_from_csv(db: Session, content: str, broker: str):
    ...
    for row in reader:
        ...
        try:
            ...
            trade = db.query(Trade).filter(Trade.symbol == symbol, Trade.status == "open").first()
            if not trade:
