...
SUMMARY_GENERATORS: Dict[str, Callable[[Dict[str, Any]], str]] = {
    "pre_market_volume_spike": lambda ind: (
        f"{ind.get('volume_spike_ratio', 0):.1f}x volume spike, {ind.get('gap_pct', 0):+.1f}% gap"
    ),
    ...
    "oversold_bounce": lambda ind: (
        f"RSI cross ({ind.get('rsi_2', 0):.0f}/{ind.get('rsi_5', 0):.0f}), ATR ${ind.get('atr_target', 0):.2f}"
    ),
...
SEVERITY_CALCULATORS: Dict[str, Callable[[Dict[str, Any]], str]] = {
    "pre_market_volume_spike": lambda ind: (
        "high"
        if ind.get("volume_spike_ratio", 0) > 5
        else "medium"
        if ind.get("volume_spike_ratio", 0) > 3
        else "low"
    ),
    "liquidity_hunt_pre": lambda ind: (
        "high"
        if (ind.get("session_volume_ratio") or 0) > 8
        or (
            ind.get("session_volume_ratio") is None
            and (ind.get("session_volume_pct_of_daily") or 0) > 0.50
        )
        else "medium"
        if (ind.get("session_volume_ratio") or 0) > 4
    ...
    "oversold_bounce": lambda ind: "high" if ind.get("rsi_2", 100) < 10 else "medium",
    "live_volume_spike": lambda ind: (
        "high"
        if ind.get("volume_spike_ratio", 0) > 8
        else "medium"
        if ind.get("volume_spike_ratio", 0) > 4
        else "low"
    ),
    "live_price_move": lambda ind: (
        "high"
        if abs(ind.get("price_move_pct", 0)) > 5
        else "medium"
        if abs(ind.get("price_move_pct", 0)) > 2
        else "low"
    ),
    "social_callout": lambda ind: (
        "high"
        if ind.get("confidence", 0) > 0.9
        else "medium"
