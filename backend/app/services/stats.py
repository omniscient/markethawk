...
class StatsService:
    ...
    def get_scorecard(
        ...
        wins = [s for s in complete if s.eod_pct_change is not None and float(s.eod_pct_change) > 0]
        win_rate = round(len(wins) / complete_count * 100, 2) if complete_count else None
        ...
        eod_vals = [float(s.eod_pct_change) for s in complete if s.eod_pct_change is not None]
        ...
        ratio = round(abs(avg_mfe / avg_mae), 4) if avg_mfe and avg_mae and avg_mae != 0 else None
        avg_r = round(sum(r_vals) / len(r_vals), 4) if r_vals else None

        avg_win = sum(v for v in eod_vals if v > 0) / max(len([v for v in eod_vals if v > 0]), 1) if eod_vals else 0
        avg_loss = abs(sum(v for v in eod_vals if v < 0) / max(len([v for v in eod_vals if v < 0]), 1)) if eod_vals else 0
        wr_frac = len(wins) / complete_count if complete_count else 0
        expectancy = round(wr_frac * avg_win - (1 - wr_frac) * avg_loss, 4) if eod_vals else None
        profit_factor = round(
            sum(v for v in eod_vals if v > 0) / abs(sum(v for v in eod_vals if v < 0)), 4
        ) if eod_vals and sum(v for v in eod_vals if v < 0) != 0 else None
        ...
        query = (
            db.query(
                *grp,
                ...
                func.sum(
                    sa.case(
                        (and_(ScannerOutcomeSummary.eod_pct_change != None, ScannerOutcomeSummary.eod_pct_change > 0), 1),
                        else_=0,
                    )
                ).label('wins'),
            )
    ...
    def get_interval_performance(
        ...
        query = (
            db.query(
                ...
                func.sum(
                    sa.case(
                        (and_(ScannerOutcomeSnapshot.pct_change != None, ScannerOutcomeSnapshot.pct_change > 0), 1),
                        else_=0,
                    )
                ).label('wins'),
            )
    ...
    def get_distribution(
        ...
        allowed = {"mfe_pct", "mae_pct", "r_multiple", "eod_pct_change", "mfe_mae_ratio"}
    ...
    def get_signals(
        ...
        base = (
            ...
            .outerjoin(ScannerOutcomeSummary, ScannerEvent.id == ScannerOutcomeSummary.scanner_event_id)
            .filter(ScannerEvent.scanner_type == scanner_type)
        )
        if start_date:
        ...
        for event, summary in rows:
            signals.append({
                ...
                "opening_price": float(event.opening_price) if event.opening_price else None,
                "previous_close": float(event.previous_close) if event.previous_close else None,
                "closing_price": float(event.closing_price) if event.closing_price else None,
                "reference_price": float(summary.reference_price) if summary and summary.reference_price else None,
                "mfe_pct": float(summary.mfe_pct) if summary and summary.mfe_pct is not None else None,
                "mae_pct": float(summary.mae_pct) if summary and summary.mae_pct is not None else None,
                "eod_pct_change": float(summary.eod_pct_change) if summary and summary.eod_pct_change is not None else None,
                "follow_through": summary.follow_through if summary else None,
                "mfe_mae_ratio": float(summary.mfe_mae_ratio) if summary and summary.mfe_mae_ratio is not None else None,
