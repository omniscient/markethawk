    ...
    try:
        while True:
            ...
            try:
                for key in sync_keys:
                    ...
                    if len(parts) >= 2 and parts[1].isdigit():
                        ...
                        pending = sum(
                            1
                            for tid in task_ids
                            if AsyncResult(tid, app=celery_app).state
                ...
                while True:
                    cursor, keys = await redis_client.scan(
                        cursor=cursor, match="ticker:*:sync", count=100
                    )
                ...
                while True:
                    cursor, keys = await redis_client.scan(
                        cursor=cursor, match="scan:*:range", count=100
                    )
                ...
                for key in uscan_keys:
                    ...
                    try:
                        ...
                        pass
                    universe = (
                        db.query(StockUniverse).filter(StockUniverse.id == uid).first()
                    )
                    universe_name = universe.name if universe else f"Universe {uid}"
                    day_idx = (
                        udata.get("day_index", 0) if isinstance(udata, dict) else 0
                    )
                    total_days = (
                        udata.get("total_days", 0) if isinstance(udata, dict) else 0
                    )
                    active_tasks.append(
                        {
                ...
                stale_cutoff = datetime.now(timezone.utc).replace(
                    tzinfo=None
                ...
                for report in quality_reports:
                    ...
                    name = (
                        universe.name if universe else f"Universe {report.universe_id}"
                    )
