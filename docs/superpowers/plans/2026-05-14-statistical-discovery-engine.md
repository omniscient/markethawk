...
**Important**: Open the generated migration file and verify the `upgrade()` function creates `signal_analysis_runs` before `signal_clusters` and both before the `signal_cluster_id` column is added to `scanner_events`. Alembic does not guarantee FK-safe ordering in autogenerate output. The correct order inside `upgrade()` must be:

```python
# 1. Create signal_analysis_runs first (no foreign keys)
op.create_table('signal_analysis_runs', ...)

# 2. Create signal_clusters (FK → signal_analysis_runs)
op.create_table('signal_clusters', ...)

# 3. Add signal_cluster_id column to scanner_events (FK → signal_clusters)
op.add_column('scanner_events', sa.Column('signal_cluster_id', sa.Integer(),
    sa.ForeignKey('signal_clusters.id'), nullable=True))
op.create_index('ix_scanner_events_signal_cluster_id', 'scanner_events', ['signal_cluster_id'])
```

If the autogenerated file has these operations in a different order, reorder them manually before running `alembic upgrade head`.
