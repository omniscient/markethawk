...
```bash
# The spec freezes this exact content:
...
3. If the file content matches exactly: proceed to Task 2. If it differs: note the deviation but do not modify — the spec frozen content is the source of truth. Any deviation must be raised as a spec discussion, not silently corrected by the plan.
...
```bash
cd frontend && npx vitest run src/components/ui/PageLoader.test.tsx
```

Expected output:
```
 ✓ src/components/ui/PageLoader.test.tsx (2)
   ✓ PageLoader > renders without crashing
   ✓ PageLoader > renders a spinning element

 Test Files  1 passed (1)
 Tests       2 passed (2)
```

2. Confirm that both tests pass with exit code 0. If either test fails, record the failure — do not attempt a fix within this task's scope (that would be new scope).
...
3. If thresholds are met: proceed to Task 4.

4. If any threshold fails: do **not** modify the thresholds or add more tests — that is new scope. Record the failure and flag it in the issue comment.
...
**Files:** `docs/superpowers/plans/2026-06-12-pageloader-test-scope-correction.md`
...
2. Stage and commit the plan file:

```bash
git add docs/superpowers/plans/2026-06-12-pageloader-test-scope-correction.md
git commit -m "plan: PageLoader test scope correction (#311)"
```

Expected output:
```
[refine/issue-311-test-frontend---scope-correct-pageloader <sha>] plan: PageLoader test scope correction (#311)
 1 file changed, ...
```
...
| 4 | TypeScript gate + commit plan | plan doc | yes |

**Total:** 4 tasks, 10 steps. No production code changes. No test changes. One commit (plan doc only).
