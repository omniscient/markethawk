...
| `backend/tests/services/test_quality_gate_service.py` | 10 unit tests — all against pure builder, no DB fixture |
...
#### 1e. Verify the shape test passes
...
#### 2a. Add all 10 behavioral unit tests

Replace the test file contents with the full 10-scenario suite. The `test_assessment_shape` function written in Task 1 stays at the end:
...
#### 2b. Verify all tests fail (10 failures)
...
git commit -m "feat(services): QualityGateService pure builder with 10 unit tests (#492)"
```

---

### Task 3 — DB-Aware Wrapper Smoke Test

**Files**: `backend/tests/services/test_quality_gate_service.py`

#### 3a. Add wrapper smoke test

Append to `backend/tests/services/test_quality_gate_service.py`:

```python
# ── DB wrapper smoke test ─────────────────────────────────────────────────────
...
#### 3c. Run the broader service tests to confirm no regressions
...
| 2 | `feat(services): QualityGateService pure builder with 10 unit tests (#492)` | `services/quality_gate_service.py`, `tests/…/test_quality_gate_service.py` |
...
- [ ] `backend/tests/services/test_quality_gate_service.py` — 15 tests, all green
