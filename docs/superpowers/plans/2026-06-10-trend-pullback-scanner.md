...
**Arch:** New service `trend_pullback_scan.py` (modeled on `pocket_pivot.py`), wired into the existing scan task, event-helper, and scheduler infrastructure. One expansion to `OutcomeService`. One Alembic seed migration.  
**Tech Stack:** FastAPI backend · SQLAlchemy (sync `db.query()` inside `async def` — the established scanner pattern, same as `pocket_pivot.py`) · pandas · pytest · Alembic
...
| `backend/app/alembic/versions/<hash>_seed_trend_pullback_scanner_config.py` | **Create** — scanner_configs row (down_revision = `c7d8e9f0a1b2`, the single current head) |
...
Run and verify it **fails** (AttributeError or assertion failure):
```bash
...
In `backend/app/services/outcome_service.py`, around line 92, add the `"10d"` entry to `interval_map` inside `capture_snapshot`:
...
Refs #299 — trend_pullback is a 3-10 day setup that needs a 10d outcome
horizon beyond the existing 5d maximum."
```
...
summ = generate_event_summary('trend_pullback', ind)
print('severity:', sev, '— expected: high')
print('summary:', summ)
assert sev == 'high', f'Expected high, got {sev}'
"
# Expected: AssertionError (returns 'medium' fallback, not 'high')
```

### Step 2.2 — Implement

In `backend/app/services/event_helpers.py`, add entries to both dicts. Place them after the `"oversold_bounce"` entry:
...
ind_medium2 = {'pullback_depth_pct': 6.0, 'rsi5': 32.0}   # depth ok but RSI >= 30
...
Refs #299 — severity=high when depth ≤ 8% and RSI5 < 30, medium otherwise."
```
...
Tests build pandas-compatible bar fixtures (MagicMock objects with .open/.high/.low/.close/.volume)
and verify the scanner fires or suppresses correctly.
"""
...
def _bar(
    ...
    b.volume = volume
    # timestamp: NYSE open UTC for _EVENT_DATE (not checked in the scan loop)
...
def _pullback_series(n_history: int = 215, pullback_bars: int = 7) -> list:
    ...
    Pullback phase (last pullback_bars):
      - First (pullback_bars-1) bars: close slightly above SMA20, with high
        at 20-day-swing-high level.  Simulates orderly retreat.
    ...
    This ensures:
      ...
      - RSI(5) drops on the retreating bars → < 40
      - All closes > SMA50 (no breakdown)
    """
    # Build 220 bars of rising prices, then flatten / dip the last pullback_bars
    base = _rising_series(n_history)
    peak_close = base[-1].close  # e.g. $71.40 at bar 214

    # Pullback: drop ~5% over pullback_bars, low touching SMA20 on the last bar.
    # SMA20 of the last bar ≈ avg of bars[-19:] = avg of declining closes.
    # We set close of last bar to peak * 0.95 and ensure low = close * 0.990
    # (below SMA20 * 1.01 boundary at the exact tolerance).
    pullback_closes = [
    ...
    for i, c in enumerate(pullback_closes):
        ...
        b.close = c
        b.open = c * 1.001
        # high is elevated for the first 3 bars (swing high formation)
        b.high = (peak_close * 1.002) if i < 3 else c * 1.002
        # last bar: low dips to ~SMA20
...
def test_clean_signal_fires():
    ...
    assert "rsi5" in ind
    assert "pct_off_252d_high" in ind
    assert "pullback_depth_pct" in ind
    assert "consecutive_days_above_sma20" in ind
    assert "atr14" in ind
...
# ---------------------------------------------------------------------------
# Scenario 2: Insufficient history (< 210 bars) — counted as no_data
# ---------------------------------------------------------------------------
...
def test_downtrend_close_below_sma50_does_not_fire():
    # Build a series where prices DROP dramatically at the end so close < SMA50
    bars = _rising_series(220, start=100.0, step=0.1)
    # Overwrite last bar with a far-below-SMA50 close
    last = bars[-1]
    last.close = 50.0   # SMA50 ≈ 100 + ~170*0.1/2 = much higher
    last.low = 49.5
    last.high = 51.0
    results, diag, _ = _run_scan(bars)
    assert len(results) == 0
...
def test_sma50_below_sma200_does_not_fire():
    # Build a series that FALLS over time → SMA50 ends up below SMA200
    bars = [_bar(200.0 - i * 0.2) for i in range(222)]  # falling from $200
    results, diag, _ = _run_scan(bars)
    assert len(results) == 0
...
def test_sma50_not_rising_does_not_fire():
    # Build 200 bars rising, then 22 bars FLAT so SMA50 stops rising
...
def test_not_near_highs_does_not_fire():
    # 252-bar rising series to establish the 252d high, then sharp decline (>15%)
    bars = _rising_series(252, start=100.0, step=0.10)  # peak ~$125
    # Add 10 declining bars so close is ~20% off the high
    for _ in range(10):
        bars.append(_bar(bars[-1].close * 0.978))  # ~2.2% drop per bar (10 bars = ~20%)
    results, diag, _ = _run_scan(bars)
    assert len(results) == 0
...
def test_less_than_5_days_above_sma20_does_not_fire():
    # Override: force only 3 consecutive prior closes above SMA20 by using
    # a config with min_days_above_sma=5 and bars where only 3 prior closes
    # are above SMA20 (the rest are below it).
    bars = _pullback_series(n_history=215, pullback_bars=3)  # only 3 above SMA20 in history
    results, diag, _ = _run_scan(bars, config={"min_days_above_sma": 5})
    # With only 3 pullback bars (= 3 consecutive closes above SMA20), criterion fails
    assert len(results) == 0
...
def test_pullback_too_shallow_does_not_fire():
    # Last bar close only 1% below 20-day swing high → depth < pullback_min_pct
    base = _rising_series(220)
    peak = base[-1].close
    pb_bar = MagicMock()
    pb_bar.close = peak * 0.99  # 1% below swing high
    pb_bar.open = peak * 0.995
    pb_bar.high = peak * 1.001
    pb_bar.low = base[-5].close * 0.998  # low tags some level (not exact)
    pb_bar.volume = 600_000
    pb_bar.timestamp = datetime(2026, 1, 15, 14, 30, 0)
    bars = base[:-1] + [pb_bar]  # replace last bar
    results, diag, _ = _run_scan(bars, config={"pullback_min_pct": 3, "pullback_max_pct": 12})
    # depth ~1% < 3% → doesn't fire
    assert len(results) == 0
...
def test_pullback_too_deep_does_not_fire():
    ...
    peak = base[-1].close
    pb_bar = MagicMock()
    pb_bar.close = peak * 0.85  # 15% below swing high
...
def test_rsi_too_high_does_not_fire():
    # A series that only goes up (RSI stays high) — no pullback → RSI5 near 100
    bars = _rising_series(222, step=0.50)  # steep uptrend keeps RSI high
    results, diag, _ = _run_scan(bars, config={"rsi_max": 40})
    # With a steep rising series, RSI5 stays well above 40
    assert len(results) == 0
...
def test_low_dollar_volume_does_not_fire():
    # Use volume=1_000 (very low) so dollar vol far below $5M threshold
    bars = _pullback_series()
    for b in bars:
        b.volume = 1_000
...
def test_below_price_floor_does_not_fire():
    # Build series where last bar close = $3.50 (below $5 floor)
...
# ---------------------------------------------------------------------------
# Scenario 13: Severity = high (depth ≤ 8% AND RSI5 < 30)
# ---------------------------------------------------------------------------
def test_severity_high_when_deep_pullback_with_low_rsi():
    bars = _pullback_series()
    results, diag, mock_save = _run_scan(bars)
    if len(results) == 1:
        ind = mock_save.call_args.kwargs["indicators"]
        depth = ind.get("pullback_depth_pct", 100)
        rsi5 = ind.get("rsi5", 100)
        from app.services.event_helpers import compute_event_severity
        expected_sev = "high" if (depth <= 8 and rsi5 < 30) else "medium"
        assert ind.get("severity") == expected_sev, (
            f"severity mismatch: depth={depth}, rsi5={rsi5}, got {ind.get('severity')!r}"
        )
...
> All public functions are `async def` (same as `pocket_pivot.py` entrypoints) so they integrate with the orchestrator and `scanner_map`. DB access uses sync `db.query()` inside `async def` — the established scanner pattern that the codebase relies on.
...
async def run_trend_pullback_scan(
    ...
    For each (ticker, event_date):
      ...
      4. Persist ScannerEvent if all criteria pass.
    """
    ...
    for event_date in trading_days:
        for ticker in tickers:
            try:
                ...
                # Need enough bars for SMA200 to be valid on the last row
                if len(bars) < 210:
                    counts["no_data"] += 1
                    continue
                ...
                sma_fast = int(cfg["trend_sma_fast"])   # 50
                sma_slow = int(cfg["trend_sma_slow"])   # 200
                sma_pull = int(cfg["pullback_sma"])     # 20
                rsi_period = int(cfg["rsi_period"])     # 5
                ...
                # 252-day rolling high; 20-day swing high
                ...
                sma_rising_lb = int(cfg["sma_rising_lookback"])  # 20
                ...
                # Count consecutive prior bars with close > sma20
                ...
                # No close below SMA50 in the 20-bar pullback window
...
# Expected: 15 passed (some may show skipped if the clean_signal_fires scenario
# doesn't fire with the fixture — adjust fixture parameters if needed; see note below)
```

> **Fixture tuning note:** If `test_clean_signal_fires` fails because the fixture bars don't produce all-True criteria, adjust `_pullback_series` parameters. Specifically:
> - If `pullback_in_progress=False`: increase `pullback_bars` to ensure ≥5 consecutive closes above SMA20 exist before the tag
> - If `orderly=False` (depth < 3%): increase the per-bar drop in `pullback_closes`
> - If `rsi_reset=False` (RSI5 ≥ 40): add more declining bars to the series
...
**Edit A — `_run_range_scan_logic`: add to `scanner_map`**
...
Add the import at the top of `_run_range_scan_logic`:

```python
    from app.services.trend_pullback_scan import run_trend_pullback_scan_for_date as _tp_scan
```

**Edit B — `_run_universe_scan_logic`: add lazy import**
...
After the `run_pocket_pivot_scheduled` task (around line 687), add the analogous task:
...
# Expected: no import errors; backend started successfully
```
...
git commit -m "feat(tasks): wire trend_pullback into scanner_map and scheduled tasks

Adds run_trend_pullback_scheduled Celery task (mirrors run_pocket_pivot_scheduled),
imports service for orchestrator self-registration, adds 'trend_pullback' to
_BEAT_SCHEDULED_SCANNER_TYPES for startup validation.
Refs #299."
```
...
**Files:**  
`backend/app/alembic/versions/<hash>_seed_trend_pullback_scanner_config.py`

> **Note:** The migration tree has a **single head** (`c7d8e9f0a1b2`). No merge migration is needed. The seed migration targets `c7d8e9f0a1b2` directly as `down_revision`.
...
### Step 5.2 — Create seed migration manually

```bash
docker-compose exec backend python -m alembic revision \
  -m "seed_trend_pullback_scanner_config"
# Expected: creates backend/app/alembic/versions/<hash>_seed_trend_pullback_scanner_config.py
...
revision: str = "<hash>"              # filled by alembic
...
# Expected: Running upgrade ... -> <hash>, seed_trend_pullback_scanner_config
```
...
### Step 5.4 — Verify scanner appears in UI dropdown endpoint
...
Row seeded with is_active=true (no repeat of the pocket_pivot is_active=false bug),
down_revision=c7d8e9f0a1b2 (single current head). Includes name, description,
criteria='[]', parameters, outcome_config (intervals 1d/2d/5d/10d), data_requirements,
universe_id=1 with CI-safe ON CONFLICT DO NOTHING guard.
Refs #299."
```
...
# Expected: full config JSON with is_active=true, parameters dict, outcome_config with 10d
```

### Step 6.3 — Trigger a historical single-ticker scan (verify scan runs without error)

```bash
# Replace AAPL and dates with tickers and dates that have daily bar data in the DB
# The scan should complete and return 0 or more events without a 5xx error
...
# Expected: HTTP 200, task_id in response
```
...
```bash
docker-compose exec backend python -m pytest backend/tests/services/test_trend_pullback_scan.py \
...
|---|---|---|---|
| 1: 10d interval | `outcome_service.py` | 4 | `interval_map["10d"]` recognised |
...
| 4: task wiring | `scanning.py` | 4 | scanner_map, scheduled task, beat list |
| 5: migration | 1 alembic file | 5 | scanner_configs row, is_active=true, down_revision=c7d8e9f0a1b2 |
| 6: validation | live | 6 | CLAUDE.md compliance |

**Total: 6 tasks, 28 steps**

---

## Memory: Patterns Applied

- **[AVOID] sync SQLAlchemy async patterns**: scanner uses sync `Session` + `db.query()` — same as `pocket_pivot.py` and `oversold_bounce_scan.py`. The AsyncSession [AVOID] pattern applies to API routes, not daily scanner services.
- **[PATTERN] `__init__.py` import not required**: `trend_pullback_scan.py` self-registers at import time via `register(...)` at module level; no model additions, so no `__init__.py` edit needed.
