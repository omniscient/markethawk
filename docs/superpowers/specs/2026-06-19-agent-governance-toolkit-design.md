...
**Date:** 2026-06-19 (revised; originally 2026-06-01)
**Issue:** #148
...
**Runtime monitoring of dark-factory execution (deferred to a follow-up issue).**
AGT operates at governance/review time — it intercepts intent before execution. It does not monitor what agents do *during* a run: agent loops within a single run, silent failures (empty output on a 200 response), or runaway token cost mid-run are outside its scope and outside the scope of this issue. The dark factory does have coarse terminal guards (scheduler `max_retries: 3`, `factory_wip_limit: 1`, post-run cost report) but lacks in-run detection. This is a real gap, addressed in a separate follow-up issue using existing MarketHawk observability infrastructure (Prometheus + Grafana + Seq). See follow-up issue for runtime observability scope.
...
### D. Third-party runtime observability SaaS alongside AGT

A comment on this issue suggested pairing AGT with a commercial SaaS runtime monitoring product. This is out of scope for v1 and is not recommended: (1) runtime monitoring is a distinct concern from governance/review-time policy enforcement and belongs in a separate issue; (2) MarketHawk already runs Prometheus, Grafana, and Seq — any runtime observability for dark-factory execution should use existing in-house infrastructure rather than an external SaaS dependency with unknown data-egress and vendor-relationship implications. See Non-goals section and follow-up issue.
...
- Runtime observability of dark-factory execution (loop detection, silent failure alerting, real-time cost tracking) is explicitly out of scope for this issue. See Non-goals section.
