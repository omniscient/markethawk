# Dispatch Ceiling Quarterly Revisit — Process Design
...
routinely fails (false negatives). This spec defines the process for the first quarterly
reassessment, which runs at or after 2026-09-12.

---

## Requirements

1. Pull Factory Scorecard success-by-size (S/M/L) numbers for the window 2026-06-12 → 2026-09-12.
...
4. Produce a recommended `.archon/.env` diff as a PR for human review — the agent does NOT mutate
   `.archon/.env` directly.
5. File the next quarterly revisit issue (~2026-12-12) unconditionally, to keep the cadence
...
python3 scripts/fetch_scorecard.py \
  --since 2026-06-12 \
  --until 2026-09-12 \
  --output /tmp/scorecard-q1-2026.json
```
...
**Adding a new keyword** requires the inverse evidence: ≥5 M-size `closed` PRs whose titles share
...
Generate a report as a Markdown comment on issue #355 containing:
...
- If any changes are warranted: open a PR that modifies only the `ABOVE_CEILING_KEYWORDS` line in
  `.archon/.env`. The PR description references issue #355 and the decision data.
- If no changes: note "no changes warranted this quarter" and close with that comment.

### Step 6 — File the next quarterly revisit issue
...
- Prior revisit: #355 (comment with Q3 results)

## Target date

**2026-12-12** (quarterly from 2026-09-12).
The primary trigger is #331 producing per-bucket success rates over a full quarter;
2026-12-12 is the time-boxed backstop if the data hasn't converged.

---
*Filed automatically by issue #355 revisit agent*
...
quarterly. Rejected. The CLI-script + quarterly-issue pattern is the correct separation.
...
- The M-bucket will have n ≥ 5 PRs by 2026-09-12 (3 months of activity). If it doesn't, the spec's
  thresholds still apply per-keyword; ambiguous/insufficient-data decisions default to "no change."

---

## Open Questions (non-blocking)

1. **Minimum total window PRs** — if the factory dispatched very few M tickets in Q3 (say < 10
   total), is the analysis meaningful? The n ≥ 5 per-keyword guard handles the worst case;
   overall M_baseline is still usable as a reference even at small n.
...
3. **DISPATCH_CEILING_ENABLED kill-switch** — if the ceiling was disabled at any point during Q3,
   blocked tickets may have been dispatched, changing the cohort composition. The revisit agent
   should check `gh issue list --label above-ceiling` to estimate how much the ceiling actually
...
- [ ] Run `python3 scripts/fetch_scorecard.py --since 2026-06-12 --until 2026-09-12 --output /tmp/scorecard.json`
