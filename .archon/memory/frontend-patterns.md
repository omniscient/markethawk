...
## Frontend: AI-Generated Content Visual Treatment

- [PATTERN] Use purple accent (`bg-purple-900/20 border-purple-700/30`, `text-purple-400`) for AI-generated content containers, paired with the `Sparkles` lucide-react icon and an "AI-generated" badge label. Purple is the codebase's "derived/computed" accent (also in `MetricCard.tsx`, `EdgeExplorer.tsx`) and does not collide with severity (red/yellow/blue), regime (green/red/amber/blue), or quality-score (green/yellow/gray) semantics. Deterministic content keeps standard `bg-gray-800/40 border-gray-700/50` styling. <!-- issue:#482 date:2026-06-19 expires:2026-12-19 source:refine -->
- [AVOID] Do not use icon-only visual distinction for AI-generated content — pair the Sparkles icon with an "AI-generated" text label so the distinction is accessible to screen readers and colorblind users. <!-- issue:#482 date:2026-06-19 expires:2026-12-19 source:refine -->
- [PATTERN] `Sparkles` from lucide-react is not currently imported anywhere in the frontend codebase (confirmed grep on 2026-06-19). Import it explicitly in any component that needs it rather than assuming it's available. <!-- issue:#482 date:2026-06-19 expires:2026-12-19 source:refine -->
