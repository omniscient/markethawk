...
- [FIX] When CI `migration-check` fails with "relation X does not exist", check whether a `git merge origin/main` accidentally overwrote guarded migration stubs with their unguarded autogenerated versions; run `git diff origin/main HEAD -- backend/app/alembic/versions/` to identify files that diverged, then restore them to match `main` using `git show origin/main:<path>`. <!-- issue:#370 date:2026-06-13 expires:2026-12-13 source:implement -->
---
