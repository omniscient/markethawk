...
- [PATTERN] Use `ensure_utc(dt)` from `app.utils.time` to stamp a naive datetime with UTC tzinfo (returns tz-aware). Distinct from `to_utc_naive` (returns naive): `ensure_utc` is for .replace(tzinfo=timezone.utc) sites; `to_utc_naive` is for .astimezone(timezone.utc).replace(tzinfo=None) sites. Both are idempotent. <!-- issue:#632 date:2026-06-26 expires:2026-12-26 source:refine -->
- [AVOID] Do not introduce a `local_to_utc` alias for `to_utc_naive` — the _naive suffix signals the return type and distinguishes it from `ensure_utc` (tz-aware). Adding the alias for the sake of the issue's naming creates a second name for one behavior and invites confusion. <!-- issue:#632 date:2026-06-26 expires:2026-12-26 source:refine -->
