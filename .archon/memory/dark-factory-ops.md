...
- [PATTERN] Memory entry metadata uses two orthogonal axes: source: records the pipeline phase (refine/implement/conformance/code-review); agentId: records the role identity from the 13-value vocabulary in agent_roles.sh. Do not conflate them or require a 1:1 mapping — a phase can write under multiple agentIds and the same agentId can appear across phases. <!-- issue:#651 date:2026-06-27 expires:2026-12-27 source:refine -->
- [PATTERN] Role-ID constants for Dark Factory memory scoping live in dark-factory/scripts/agent_roles.sh, not in gate_lib.sh. gate_lib.sh sources agent_roles.sh at the top. This keeps gate_lib.sh to its stated three-primitives contract and lets non-gate commands source the role vocabulary without pulling in gate writer functions. <!-- issue:#651 date:2026-06-27 expires:2026-12-27 source:refine path:dark-factory/scripts/ -->
