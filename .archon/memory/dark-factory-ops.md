...
- [FIX] `.github/workflows/ci.yml`'s `adapter-validate` job ("Factory adapter validation") must `pip install pyyaml aiohttp` — `factory_core.adapter` transitively imports `factory_core/model_proxy.py`, which requires `aiohttp` upstream in omniscient/dark-factory (their Dockerfile installs both together for dark-factory#208); a pyyaml-only install breaks with `ModuleNotFoundError: No module named 'aiohttp'` as soon as that upstream dependency lands, and it fails for every PR, not just the one touching this file. <!-- issue:#50 date:2026-07-17 expires:2027-01-17 source:implement -->
