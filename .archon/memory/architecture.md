...
## Codeindex Artifacts (issue #200)

- [PATTERN] When codeindex artifacts (codeindex.json, symbolindex.json, docs/codeindex-hotspots.md) are absent from the repo, generate and commit them using `codeindex analyze .`, `codeindex symbols .`, and `codeindex high-blast > docs/codeindex-hotspots.md`. This creates a baseline the factory's regen-codeindex node can update. Stale-but-present is strictly better than absent for MCP tool availability. <!-- issue:#200 date:2026-06-05 expires:2026-12-05 source:refine -->

- [AVOID] Do not fix "missing artifact" issues with documentation-only changes when the generator is available in the factory environment. The correct fix is to generate the baseline and update the docs to accurately describe the model (factory-regenerated, may lag HEAD, advisory rather than mandatory). <!-- issue:#200 date:2026-06-05 expires:2026-12-05 source:refine -->
