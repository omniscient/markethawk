...
- [PATTERN] Use a split tsconfig layout for the frontend: `tsconfig.json` as a references-only root (no `include`/`compilerOptions`), `tsconfig.app.json` for production code (excludes `**/*.test.*`, `src/test-utils`, `src/test-setup.ts`), `tsconfig.node.json` for `vite.config.ts`. The build script uses `tsc -p tsconfig.app.json && vite build`. This matches the canonical Vite scaffold and keeps the production typecheck environment free of test globals. <!-- issue:#193 date:2026-06-05 expires:2026-12-05 source:refine -->

- [AVOID] Do not add `"types": ["vitest/globals", "@testing-library/jest-dom"]` to the production `tsconfig.json`. It injects test-runner globals (`describe`, `it`, `vi`, etc.) into production type scope, allowing test utility calls in production code to pass type-checking silently. <!-- issue:#193 date:2026-06-05 expires:2026-12-05 source:refine -->
