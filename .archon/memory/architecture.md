...
- [PATTERN] Each extension point (providers, scanners, alert channels, broker adapters) owns its own per-domain `ExtensionRegistry[T]` instance (`app.core.extensions`, #439). The shared `ExtensionRegistry` primitive is instantiated separately per subsystem — not as a global multi-type singleton keyed by `(type, name)`. Precedents: `_REGISTRY: ExtensionRegistry[ScannerDescriptor]` in `scan_orchestrator.py` (#440) and `DataProviderFactory._registry: ExtensionRegistry[BaseDataProvider]` (#441). `_SCREENER_REGISTRY` in `discovery_service.py` is a plain dict keyed by asset class and is not an extension point. <!-- issue:#441 date:2026-06-15 expires:2026-12-15 source:refine -->
...
- [PATTERN] When migrating a self-describing object registry (like `DataProviderFactory._providers`) to wrap `ExtensionRegistry`, use composition over inheritance: keep the domain-specific class with its classmethods and project provider-specific fields (`is_available`, `supported_asset_classes`) inside those methods; if the stored object has no `.key`, add a concrete `key` property rather than a wrapper descriptor. Extending `ExtensionRegistry` directly would require overriding most methods and force provider concepts into the generic primitive. <!-- issue:#441 date:2026-06-15 expires:2026-12-15 source:refine -->
