...
- [PATTERN] In the write-through memory adapter (memory_write.py), always write to .archon/memory/*.md first (markdown is the source of truth), then POST to agentmemory as a best-effort step — agentmemory failures log a WARNING but do not affect the exit code. Skip agentmemory when the markdown write is a no-op (dedup hit or cap reached) to keep both stores consistent. <!-- issue:#648 date:2026-06-27 expires:2026-12-27 source:refine -->
