...
- [PATTERN] Emit memory-trace.json from inside memory_retrieve.py via a --emit-trace-to <path> flag, not from a second gate invocation — the single in-process retrieval call that generates the memory context also knows exactly what it selected, guaranteeing the trace matches what entered context. <!-- issue:#647 date:2026-06-30 expires:2026-12-30 source:refine -->
- [AVOID] Do not emit the memory trace from a second call to memory_retrieve.py after the first (gate-side second-pass pattern) — two invocations create a count-mismatch window where the trace could misrepresent what the agent actually received, defeating the auditability goal. <!-- issue:#647 date:2026-06-30 expires:2026-12-30 source:refine -->
