...
- [PATTERN] When a documentation defect and its corresponding machine-generated memory file entry contain the same false claim, fix the memory file as part of the same issue — it is the most dangerous location because agents consume it as authoritative instructions. The memory file's own preamble defers to canonical docs, so fixing both is the natural completion of a single acceptance criterion. <!-- issue:#201 date:2026-06-05 expires:2026-12-05 source:refine -->

- [AVOID] Do not defer topology diagram updates to a separate issue when the stale entries are explicitly named in the defect ticket and each new node requires only 1-2 mermaid lines derived from `depends_on` in docker-compose.yml. Deferral creates inconsistency between complete model/router tables and an incomplete topology. <!-- issue:#201 date:2026-06-05 expires:2026-12-05 source:refine -->
