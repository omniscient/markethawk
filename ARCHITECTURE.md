...
| File | Responsibility |
|------|---------------|
| `scan_orchestrator.py` | Scanner registry and orchestrator — the extension point for scanner registration. `ScannerDescriptor` frozen dataclass (`key`, `display_name`, `description`, `run`, `supports_date_range`, `asset_classes`, `default_parameters`); `_REGISTRY` is an `ExtensionRegistry[ScannerDescriptor]` (`app/core/extensions.py`) populated via `register()` at import time (`replace=True` to override an existing key, otherwise raises `ExtensionDuplicateError`). A private module registered through `load_extension_modules()` resolves and runs identically to a built-in. `run(scanner_type, tickers, db, event_date, scanner_run=None, gate_metadata=None)` is the single dispatch entry point from `tasks/scanning.py`; threads `gate_metadata` to registered scanner descriptors; unknown-key errors list all registered keys. `get_all()` enumerates entries for `GET /api/v1/scanner/types` (now including `asset_classes`/`default_parameters`). `compute_next_run(scanner_type)` returns next scheduled fire time. `get_scan_progress(redis_url, universe_id, scanner_type)` reads Redis progress state. `request_scan_cancel(redis_url, scan_id)` sets Redis cancel flag. `enqueue_scan(db, request)` creates `ScannerRun` row and dispatches Celery task. |
