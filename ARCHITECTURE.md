...
| `stats.py` | Aggregate statistics helpers for dashboard metrics. `get_scorecard()` now accepts optional `regime` filter. `get_regime_breakdown()` returns per-regime win-rate, avg MFE/MAE, and sample size. |
| `regime_service.py` | `RegimeService` — trains a `GaussianHMM` (hmmlearn) on SPY daily bars from `stock_aggregates`, selects optimal state count via BIC (2–5 states), maps state indices to human-readable labels (`risk_on`, `risk_off`, `high_volatility`, `low_vol_drift`). `train_and_persist(db)` serializes the model to PostgreSQL and caches the current regime in Redis (25-hour TTL). `get_regime_at_date(db, date)` predicts regime for any historical date. |
...
| `outcomes.py` | `/api/v1/outcomes/*` — scorecard (now accepts `?regime=` filter), intervals, distribution, edge decay, signals, event detail, backfill; `POST /analyze` (trigger analysis), `GET /correlations`, `GET /analysis/latest`, `GET /regime-breakdown/{scanner_type}` (per-regime win-rate, MFE, MAE, sample size) |
...
| `ScannerEvent` | `scanner_events` | One row per ticker that passed all criteria in a run. Carries `signal_quality_score` (Float, indexed DESC NULLS LAST) computed at write time by `signal_ranker.py`. Carries `regime` (`String(30)`, nullable, indexed) populated by `RegimeService.get_regime_at_date()` in `save_event()`. Also written by the live scanner. |
| `RegimeModel` | `regime_models` | Persists fitted GaussianHMM artifacts. `model_b64` (base64-encoded pickle), `state_label_mapping` (JSONB: `{"0": "risk_on", ...}`), `feature_set` (JSONB), `data_start_date`/`data_end_date`, `bic_score`, `status` (`active`/`archived`), `version`. Active model drives daily regime labeling. |
