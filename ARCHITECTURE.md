...
| `trend_pullback_scan.py` | Self-registers `"trend_pullback"` in the orchestrator. Detects stocks in confirmed uptrends (close > SMA50 > SMA200, SMA50 rising 20d) pulling back 3–12% to their rising 20-day SMA; RSI(5) < 40 confirms the reset; $5M+ avg dollar volume floor. Uses daily bars from `StockAggregate` with pandas rolling indicator computation. Runs nightly at 02:00 UTC via Celery beat. |
...
| `discovery_service.py` | Bulk ticker sync from Polygon: paginated reference data, rate-limit-aware batching. `run_screen()` dispatches asset-class criteria to `_SCREENER_REGISTRY`; `register_screener()` and `_apply_shared_filters()` define the extension seam. |
| `stock_screener.py` | `StockScreener.screen(db, criteria)` — applies fundamental and metric filters on `TickerReference`/`StockMetric` (inner-joins when `min_volume` is set). Self-registers as `"stocks"` in `_SCREENER_REGISTRY` at import. |
| `futures_screener.py` | `FuturesScreener.screen(db, criteria)` — resolves `futures_symbols` against `FuturesContract`; known symbols get exchange from DB, unknown symbols become placeholder rows. Self-registers as `"futures"` in `_SCREENER_REGISTRY` at import. |
...
| `alert_service.py` | `AlertRuleService` — rule matching (scanner type + severity + cooldown filters). `NotificationDispatcher` — fan-out to browser push / email / Google Chat / webhook channels; records `AlertDeliveryLog`. `_send_browser_push_generic(title, body, db, urgency, url)` — generic push helper used by both scanner alerts and `system_notifier`. `save_event()` — centralized write path for scanner events: computes severity (`low`/`medium`/`high` enforced) and validates JSONB dicts (`indicators`, `criteria_met`, `enrichment`) are JSON-serializable before persisting. Injects current HMM regime via `RegimeService.get_regime_at_date()` into the `ScannerEvent.regime` column. |
| `system_notifier.py` | `notify_system(title, body, severity, dedupe_key, channels, db)` — generic email/push delivery for non-scanner events (autopilot, circuit-breaker trips, preview failures). Email uses `OPS_ALERT_EMAIL`; skips silently if unset. Dedupe/cooldown (60 min) backed by `AlertDeliveryLog` rows with `scanner_type="system_notification"`. Fail-soft: each channel is attempted independently, failures logged but never raised. |
...
| `outcomes.py` | `/api/v1/outcomes/*` — scorecard (`include_warnings`, `include_all`, and `regime` params; returns `gate_filter`/`gate_status` counts), intervals, distribution, edge decay, signals (each item carries `gate_tier`), event detail, backfill; `GET /regime-breakdown/{scanner_type}` (per-regime win rate, avg MFE/MAE); `POST /analyze` (trigger analysis), `GET /correlations`, `GET /analysis/latest` |
...
| `alerts.py` | `GET /api/v1/alerts/stats` (dashboard header cards), `GET/POST /api/v1/alerts/rules` (list / create), `PATCH/DELETE /api/v1/alerts/rules/{id}`, `POST /api/v1/alerts/rules/{id}/test` (dry-run match against recent events), `GET /api/v1/alerts/logs` (delivery audit trail), `GET/POST /api/v1/alerts/push/vapid-key`, `GET /api/v1/alerts/push/generate-keys`, `POST /api/v1/alerts/push/subscribe`, `DELETE /api/v1/alerts/push/unsubscribe`, `POST /api/v1/alerts/infrastructure` (Grafana alerting webhook), `POST /api/v1/alerts/system` (generic system notification; requires `X-Internal-Token` header; 503 if `INTERNAL_API_TOKEN` unset, 401 on mismatch) |
...
| `scanning.py` | `run_trend_pullback_scheduled` | Beat (02:00 UTC weekdays) | Nightly trend-pullback scan over all active configs |
