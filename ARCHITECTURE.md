...
| `alert_service.py` | `AlertRuleService` — rule matching (scanner type + severity + cooldown filters). `NotificationDispatcher` — fan-out to browser push / email / Google Chat / webhook channels; records `AlertDeliveryLog`. `save_event()` — centralized write path for scanner events: computes severity (`low`/`medium`/`high` enforced) and validates JSONB dicts (`indicators`, `criteria_met`, `enrichment`) are JSON-serializable before persisting. Injects current HMM regime via `RegimeService.get_regime_at_date()` into the `ScannerEvent.regime` column. |
...
| `data_quality.py` | `POST /api/v1/data-quality/gate` — preflight trust assessment; validates a `GateRequest` (universe_id, policy, consumer, optional scope/requirements), returns `QualityGateAssessment` from `QualityGateService.assess()`; rate-limited at `SCANNER_LIMIT` (5/min) |
